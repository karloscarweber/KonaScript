...
local parser_test = Dots.Test:new("parser")

-- Test the initialization of a test parser
parser_test:add("[test_parser] Parser:new()", function(r)
  local parsey = Parser:new()
  r:_truthy(parsey, "Parser is not initialized")
  return r
end)

-- -- Test the shape of a new parser
-- parser_test:add("[test_parser] Parser:new()", function(r)
--   local env = Parser:new()
--   r:_shape(env, {values={}}, "Parser object doesn't have what we're expecting in it's shape.")
--   return r
-- end)
--
-- -- Test the definition and retrieval of variables.
-- parser_test:add("[test_parser] Parser:define(), Parser:get()", function(r)
--   local env = Parser:new()
--
